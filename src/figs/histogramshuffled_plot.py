...
initial_histogram_by_time_constant = {
    time_constant: [] for time_constant in time_constants
}
...
for filepath in glob.glob(f"{DATA_DIR}/*-results.npz"):
    ...
    seed_rng = int(results["seed_rng"])
    initial = np.load(f"{DATA_DIR}/seed={seed_rng}-initial.npz")
    initial_neuron_count_by_cluster_by_layer = initial[
        "neuron_count_by_cluster_by_layer"
    ]
    initial_histogram_by_time_constant[time_constant] += list(
        initial_neuron_count_by_cluster_by_layer.flat
    )
...
for index, time_constant in enumerate(time_constants):
    ...
    res = chisquare(
        list(histogram_merged.values()), list(expected_frequencies_merged.values())
    )
...
for index, time_constant in enumerate(time_constants):
    histogram = histogram_by_time_constant[time_constant]
    initial_histogram = initial_histogram_by_time_constant[time_constant]
    pmf = pmf_by_time_constant[time_constant]
    ...
    if significance != "":
        ax.text(
            9,
            max(np.amax(hist), np.amax(init_hist)),
            significance,
            ha="center",
            va="bottom",
        )
...
plt.savefig(f"{FIGS_DIR}/histogram-shuffled.pdf")
