...
from mynetworks.plotting import (
    ...
    get_layer_contained_sorting_from_clustering,
    plot_weights,
)
...
for index, (time_constant, learning_rate) in enumerate(
    ...
    # time = 89 if (time_constant == 10 * day and learning_rate == 0.18) else 100
    time = 1
    matrix = np.load(
    ...
    mat = plot_weights(
        ...
        get_sorted_weight_matrix(
            matrix, get_layer_contained_sorting_from_clustering(clustering, layers)
        ),
...
# plt.savefig(f"{FIGS_DIR}/robustness{time}d.pdf")
plt.savefig(f"{FIGS_DIR}/robustness{time}d_cwil.pdf")
